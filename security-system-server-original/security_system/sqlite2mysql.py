...
def main():
    import sys, os
    if len(sys.argv) < 2:
        print("Usage: python sqlite2mysql.py dbfile")
        return

    dbfile = sys.argv[1]
    if not os.path.exists(dbfile):
        print("Error: Not found the file %s" % dbfile)
        return
...
if __name__ == "__main__":
    """
    usage:
    1) sqlite3 filename.db .dump > dumpfile
    2) python sqlite2mysql.py dumpfile > mysqldumpfile
    3) mysql -u root -p dbname < mysqldumpfile
    """
    main()
