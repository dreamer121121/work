...
for line in lines:
    ...
    if Lot == "None" or Lat == "None":
        continue
    try:
        FLot = float(Lot)
        FLat = float(Lat)
    except:
        print("In " + ip + " " + Lot + " or " + Lat + " is not legal!")
        continue
    if FLot >= 180 or FLot <= -180:
        continue
    ...
    cmd = "insert into knowledgeBase_instance (name,ins_type,ip,city,country,lat,lon,port,timestamp) values('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
    ip + "_" + port + "_" + str(random.randint(0, 100)), "Camera", ip, City, Country, Lat, Lot, port,
    ...
    try:
        ...
        index = index + 1
        continue
    # conn.execute("insert into knowledgeBase_instance (name,ins_type,ip,city,country,lat,lon,port,timestamp) values('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (ip + "_" + port + "_" + str(random.randint(0,100)), "Camera", ip, City, Country, Lat, Lot, port, TimeList[index].strftime("%Y-%m-%d %H:%M:%S") ) )
    index = index + 1
f_in.close()
conn.close()
print("succeed number :" + str(su_number))
print("finish!")
"""
conn = sqlite3.connect('db.sqlite3')
